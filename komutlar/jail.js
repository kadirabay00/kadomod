...
exports.run = async(client, message, args, ayar, emoji) => {
 if (!message.member.roles.cache.has("Yetkili Rol ID") && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).addField(`${message.author} Komutu kullanmak için yetkin bulunmamakta.`)).then(m => m.delete({timeout: 7000}));
  ...
  await uye.roles.set(uye.roles.cache.has("Booster Rol ID") ? ["Cezalı Rol ID", "Booster Rol ID"] : ["Cezalı Rol ID"]).catch();
  ...
  client.channels.cache.get("Jail Log ID").send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).setDescription(`**Cezalandırıldı !**\nCezalayan Yetkili: ${message.author} (\`${message.author.id}\`)\nCezalanan Üye: <@!${uye.id}> (\`${uye.id}\`)\nCeza Sebebi: \`${reason}\``)).catch();
