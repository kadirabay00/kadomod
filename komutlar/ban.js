...
const banlog = message.guild.channels.cache.find(c => c.id === 'Ban Log ID') 
...
if(message.member.roles.highest.position <= kullanici.roles.highest.position) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Yetkililer birbirlerini banlayamazlar.`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
...
if(kullanici.id === message.guild.OwnerID) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Etiketlenen kullanıcı yasaklanamaz.`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
kullanici.ban({reason: sebep}).then(x => message.react('Mesaj Tepki ID')).catch();
...
message.channel.send(new MessageEmbed().setDescription(`${kullanici} adlı kullanıcı, ${message.author} tarafından **${sebep}** sebebi ile banlandı.`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('0x348f36').setTimestamp()) 
banlog.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('RANDOM').setTimestamp().setDescription(`**Sunucudan Yasaklandı !**\n• Yasaklayan Yetkili: ${message.author} (\`${message.author.id}\`)\n• Yasaklanan Üye: ${kullanici.user.tag} (\`${kullanici.user.id}\`)\n• Yasaklanma Sebebi: \`${sebep}\` `));
}
